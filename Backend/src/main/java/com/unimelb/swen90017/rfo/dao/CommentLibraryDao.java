...
public interface CommentLibraryDao extends BaseMapper<CommentLibraryPO> {
     ...
     * @return list
     */
    @Select("SELECT c.* FROM assessment_criteria a left join comment_library c  on a.template_element_id = c.template_element_id" +
            " WHERE a.id = #{criteriaId} AND c.delete_status = 0 order by c.comment_type desc")
